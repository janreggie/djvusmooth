...
class xdg(object):
    ...
    xdg_config_dirs = [xdg_config_home] + list(
...
class Config(object):
    def __init__(self, resource, legacy_path=None):
        ...
        for directory in xdg.load_config_paths(resource):
            self._load(os.path.join(directory, "{res}.conf".format(res=resource)))
    ...
    def del_array(self, key):
        keys_to_delete = frozenset(k for k in self._data if k.startswith(key + "["))
